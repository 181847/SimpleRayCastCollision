...
function [isCollided, collideLocation] = RayCollideCheck(ray, rayMax, collideRect, collideWorld)
    %将射线转换到碰撞体的局部坐标中.
    ...
    if max < min && max < rayMax,
        isCollided = 1;
        %计算碰撞点的世界坐标
        worldDirection = [0, 0, 1, 0] * ray;
        collideLocation = ray(4, 1:3) + max * worldDirection(1:3);
    else
        isCollided = 0;
